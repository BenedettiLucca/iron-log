...
**Current Version:** v2.4.0 (Final Release - Warm Adaptive)
...
2. **Session Flow** (`app/session/`) - Isolated stack for active workouts
   - `[routineId].tsx` - Start session (select routine)
   - `exercise.tsx` - Active exercise (sets, timer, rest)
   - `finish.tsx` - Finalize session
...
- **strength** - Weight/reps based exercises
